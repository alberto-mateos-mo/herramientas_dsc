...
```{r}
datos <- cbind(datos,m1$fitted.values)
```

```{r}
plot(m1$fitted.values,col=datos$Gender,
     main="Probabilidad de compra por género",
     ylab="probabilidad de compra")
```

```{r}
plot(m1$fitted.values,datos$Age,col=datos$Gender,
     main="Probabilidad de compra vs edad coloreado por edad",
     xlab="probabilidad de compra",ylab="edad")
```

```{r}
plot(m1$fitted.values,datos$EstimatedSalary,
...
```{r}
multi1 <- multinom(satisfied~salary)
...
```{r}
multi3 <- multinom(satisfied~salary+age)
...
```{r}
datos <- datos[default==1,]
```

__Construcción de la variable del tiempo transcurrido al default:__

```{r}
datos$meses_default <- Ddatos$duration-datos$installment_commitment
...
```{r}
D1$meses_default=D1$duration-D1$installment_commitment

m1 <- glm(meses_default ~ own_telephone, data = D1, family = Gamma)
```
